...
public class Exercises {
	...
	public boolean hasBad(String str) {
		if(str.length() >= 4 && str.substring(1,2).equals("b")) {
			if (str.substring(1, 4).equals("bad")) {
				return true;
			}
			return false;
		} else if (str.length() >= 3 && str.startsWith("b")) {
	...
	public boolean doubleX(String str) {
		...
		if (containsX && firstInstance + 1 < str.length()) {
			if (str.substring(firstInstance, firstInstance + 2).equals("xx")) {
				return true;
			}
			return false;
		}
	...
	public int last2(String str) {
		...
		if (str.length() >= 2) {
			...
			for (int i = 0; i < str.length() - 2; i++) {
				if (str.substring(i, i + 2).contentEquals(lastChars)) {
					count++;
				}
			}
		}
	...
	public String altPairs(String str) {
		...
		for(int i =0; i < str.length(); i=i+4) {
			newString += str.substring(i,i+1);
			if (str.length() > i+1) {
				newString += str.substring(i+1, i+2);
			}
		}
		return newString;
	}
